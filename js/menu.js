...
function getSlugFromUrl() {
  // 1) Kolay test için: index.html?restoran=xxx
  const qParam = new URLSearchParams(window.location.search).get('restoran');
  if (qParam) return qParam;

  // 2) 404.html'den gelen ham yönlendirme formatı: ?/restoran/xxx
  //    (adres çubuğu henüz düzeltilmemiş olsa bile burada okuyabiliyoruz)
  ...
  // 3) Normal/düzeltilmiş adres: /restoran/xxx
...
function applyTheme(settings) {
  ...
  // Animasyon stili her zaman ayarlanır (ayar yoksa varsayılan "fade")
  ...
  setTimeout(() => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        playCardEntrance(getVisibleCards());
      });
    });
  }, delay);
}
...
// ---------------------------------------------------------
// KART GİRİŞ ANİMASYONU (sırayla belirme)
// ---------------------------------------------------------

function getVisibleCards() {
  return Array.from(document.querySelectorAll('.product-card:not(.hidden-category)'));
}
...
function playCardEntrance(cards) {
  ...
  cards.forEach((card, i) => {
    ...
    // Yeniden tetiklemek için bir reflow'a ihtiyaç var
