...
function renderMenu(restaurant, categories, products, tagsByProduct) {
  ...
  if (restaurant.cover_image_url) {
    landingEl.classList.add('has-cover');
    landingEl.style.backgroundImage =
      `linear-gradient(180deg, rgba(10,8,6,0.15) 0%, var(--color-background) 96%), url('${restaurant.cover_image_url}')`;
  }
    ...
    <div class="product-card-body">
      ...
      <div class="product-card-details">
        ...
        <div class="product-card-price">
          ${product.old_price ? `<span class="product-card-old-price">${formatPrice(product.old_price)}</span>` : ''}
...
async function loadMenu() {
  ...
  const { data: restaurant, error: restaurantError } = await supabaseClient
    .from('restaurants')
    .select('id, name, slug, logo_url, cover_image_url, menu_title, is_active, instagram_url, phone_number, footer_text')
    .eq('slug', slug)
    .maybeSingle();
